...
method getWhenMessage(dDate, cTime, lEnglish) class LibUtilsObj
  ...
  if (dDate == dDataBase)
    cWhen := if(lEnglish, "Today at "+cTime, "Hoje �s "+cTime)
  else
    nDays := (dDataBase - dDate)
    if (nDays == 1)
      cWhen := if(lEnglish, "Yesterday at "+cTime, "Ontem �s "+cTime)
    else
      cWhen := if(lEnglish, AllTrim(Str(nDays))+" days ago", "H� "+AllTrim(Str(nDays))+" dias") 
    endIf
  endIf
...
Coleta propriedades de um objeto testando sua exist�ncia
...
/**
 * Retorna os vetores para o m�todo showHelp()
 */
